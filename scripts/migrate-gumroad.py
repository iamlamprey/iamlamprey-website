#!/usr/bin/env python3
"""Migrate Gumroad customers into Polar as Master Bundle grants.

The Gumroad export is a headerless email list with no per-product data (by
default scripts/orders-gumroad-test.csv; pass --full for scripts/orders-gumroad.csv),
so every address in it is treated as having purchased the
Master Bundle: for each unique email the script creates a Polar customer
(create-customer, no name) then grants the Master Bundle's legacy product with a
free subscription (create-subscription). The Master Bundle legacy product id is
read from config.json (slug "master-bundle").
...
keys.txt is looked for in the repo root or in scripts/. Run from anywhere.
    ...
    python scripts/migrate-gumroad.py --dry-run         # test CSV, no API calls
    python scripts/migrate-gumroad.py --limit 3         # test CSV, a few rows
    python scripts/migrate-gumroad.py --full            # full export
"""
...
GUMROAD_CSV_TEST = ROOT / "scripts" / "orders-gumroad-test.csv"
GUMROAD_CSV_FULL = ROOT / "scripts" / "orders-gumroad.csv"
...
def read_emails(path: Path) -> list[str]:
    ...
    # Headerless export: the first column is the email (extra columns, e.g.
    # country, are ignored). Rows without an '@' are skipped so a stray header
    # line never becomes a customer.
    emails: list[str] = []
...
def build_plan(catalog: dict, emails: list[str]) -> tuple[dict[str, dict], list[str]]:
    ...
    for email in emails:
        # No names in the Gumroad export: name is left blank (Polar only needs email).
...
def main() -> int:
    import argparse
    parser = argparse.ArgumentParser(description="Migrate Gumroad customers into Polar as Master Bundle grants.")
    ...
    emails_path = GUMROAD_CSV_FULL if args.full else GUMROAD_CSV_TEST
    print(f"source: {emails_path}")
    ...
    customers, warnings = build_plan(catalog, emails)
    ...
    if args.dry_run:
        ...
        for email, record in sorted(customers.items()):
            print(f"  {email} | name: {record['name'] or '-'} | grants: {len(record['grant_ids'])} "
                  f"-> {record['grant_ids'] or '-'}")
        return 0
